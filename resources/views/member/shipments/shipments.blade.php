{{--A shipment is a collection of items. A shipment is added by a member.
    This view generate a GUI for the member dashboard with a list of shipments added by member.--}}
...
@section('css')
    ...
    {!! Html::style('assets/dist/css/wizardMember.css') !!}
...
@section('content')
    <section class="content">
        ...
        <div class="page-head-line">
            <span>Shipments</span>
            <span class="add-new-shipment-btn padding-bottom">
    ...
    @include('member.shipments.add_shipment')
@endsection
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            var table = $("#shipments-table").DataTable({
                ...
                serverSide: false,
                retrieve: true,
                ajax: '{!! url("member/datatables/shipments") !!}',
                columns: [
            ...
            $("#add-shipment-wizard").steps({
                ...
                onStepChanging: function (event, currentIndex, newIndex) {
                    ...
                    if (currentIndex == 0) {
                        ...
                        form.validate({
                            errorPlacement: function (error, element) {
                                if (element.attr("name") == "retailer_select") {
                                    error.insertAfter(element.parent().find('span.select2:last'));
                                } else {
                    ...
                    if (currentIndex == 0) {
                        ...
                        /* Change data.retailer_name field if selec2 option is a valid id. */
                        if($('#state').select2('data')[0].id != 'add_new') {
                            data.retailer_name = $('#state').select2('data')[0].name;
                        }
                ...
                onStepChanged: function (event, currentIndex, previousIndex) {
                    if (currentIndex == 3) {
                        /* General details */
                        $('#confirmation-sender').find('span:last-child').html('<strong>' + data.retailer_name + '</strong>');
                        ...
                        $('#confirmation-vat').html("$ " + ((getShipmentSubtotal() + parseFloat(data.shipping_amount) + parseFloat(data.tax_amount)) * parseFloat('{{getSetting('VAT_TAX')}}') / 100).toFixed(2));
                    }
                },
