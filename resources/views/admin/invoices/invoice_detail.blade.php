...
@section('content')
    <div class="content">
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-6 page-head-line">Invoice detail</div>
            <div class="col-xs-12 col-sm-6 text-right">
                <a class="btn btn-primary btn-edit"
                   href="{{url('admin/invoices/' . $invoice->id . '/send-invoice')}}">
                    Send invoice
                </a>
                @if($invoice->status == 'unpaid' || $invoice->status == 'pending' )
                        <a class="btn btn-primary" href="{{ url('admin/invoices/'.$invoice->id.'/pay') }}">
                            Pay invoice
                        </a>
                    <a class="btn btn-primary btn-edit"
                       href="{{url('admin/invoices/' . $invoice->id . '/mark-as-payed')}}">
                        Mark as paid
                    </a>
                    <a class="btn btn-primary btn-edit"
                       href="{{action('Admin\InvoicesController@invalidateInvoicePayment', ['id' => $invoice->id])}}">
                        Mark as unpaid
                    </a>
                @endif
                @if($invoice->status == 'paid')
                    <a class="btn btn-primary btn-edit"
                       href="{{url('admin/invoices/' . $invoice->id . '/mark-as-unpayed')}}">
                        Mark as unpaid
                    </a>
                @endif
                @if($invoice->status != 'paid')
                    <a class="btn btn-primary" href="{{ url('admin/invoices/'.$invoice->id.'/edit') }}">
                        Edit invoice
                    </a>
                @endif
            </div>
                        ...
                        @foreach($invoice->products as $product)
                            <tr>
                                <td>
                                    ...
                                    @else
                                        <a href="{{url('admin/coupons/' . $product->coupon->id . '/edit')}}">
